...
{
    /**
     * Name of the player. Primary key
     * @since 1.0
     */
    @Id
    ...
    public int getChoice(List<Fighter> fighters) {
        ...
        else if (strategy == 2) return fighters.indexOf(worstFighter(fighters, true));
    ...
    private Fighter bestFighter (List<Fighter> fighters) {
        ...
        for (Fighter f : fighters) {
            if (f != best && f.getBase() > best.getBase())
                best = f;
     ...
     * @version 2
     */
    private Fighter worstFighter (List<Fighter> fighters, boolean trimExtras) {
        Set<Fighter> extraFighters = new HashSet<>();
        if (trimExtras) {
            while (extraFighters.size() < entityManager.extraFighters()) {
                Fighter worst = worstFighter(fighters, false);
                fighters.remove(worst);
                extraFighters.add(worst);
            }
        }

        Fighter worst = fighters.get(0);
        for (Fighter f : fighters) {
            if (f != worst && f.getBase() < worst.getBase())
                worst = f;
        ...
        if (!extraFighters.isEmpty()) fighters.addAll(extraFighters);
        return worst;
    }
    /**
    ...
    private Fighter secondBestFighter(List<Fighter> fighters) {
        ...
        if (fighters.size() == (1 + (entityManager.extraFighters())))
            return best;
